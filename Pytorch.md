...
mynetwork = MyNetwork()
optim = torch.optim.SGD(mynetwork.parameters(), lr=0.01)
...
print(vgg16_false)
```
> 新版本PyTorch不再使用pertrained参数，而使用weights参数(默认为None，不使用已经训练好的参数)

## 网络模型的保存与读取
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202409192051247.png)
```python
import torch
import torchvision
from torch import nn
...
for i in range(epoch):
    ...
    torch.save(mynetwork, "related_data/GPU_MyNetwork_{}.pth".format(i))
    print(f"模型已保存")

```

### 方式3 多卡并行DataParallel
```python
import torchvision
import torch
import time
...
# * 写法1：结合device
mynetwork = mynetwork.to(device)
mynetwork = nn.DataParallel(mynetwork, device_ids=[0,1])
# # * 写法2：结合cuda
# mynetwork = mynetwork.cuda()
# mynetwork = nn.DataParallel(mynetwork, device_ids=[0,1])
# # * 写法3：精简写法
# mynetwork = nn.DataParallel(mynetwork, device_ids=[0,1]).cuda()
# # * 写法4：GPU编号连续 
